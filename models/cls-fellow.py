...
class ClsFellow(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, max_seq_length, rate=0.1):
        ...
        self.final_layer = tf.keras.layers.Dense(2, activation=tf.keras.activations.softmax)
    ...
    def __call__(self, inp, training):
        ...
        enc_output, _ = self.encoder(inputs, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)
...
if __name__ == '__main__':
    ...
    model_selector = ModelSelector(learning_rate=0.001, batch_size=256, epochs=20, times=5)

    for f_type in fellow_types:
        for c_y in cut_year_range_dict[f_type]:
            model_selector.run_cls_model(c_y, f_type, ClsFellow, True,
